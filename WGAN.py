    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (x_, _) in enumerate(self.data_loader):
                ...
                D_real = self.D(x_)
                D_real_loss = -self.dk*torch.mean(D_real)
                ...
                if ((iter+1) % self.n_critic) == 0:
                    ...
                    self.train_hist['G_rate'].append(-G_loss.data[0])
                    G_loss.backward()
                    self.G_optimizer.step()

                    self.train_hist['D_loss'].append(D_loss.data[0])
                    self.train_hist['D_rate'].append(-D_real_loss.data[0]/2)
                if ((iter + 1) % 100) == 0:
                    print("Epoch: [%2d] [%4d/%4d] D_loss: %.8f, G_loss: %.8f D_rate: %5f G_rate :%5f" %
                          ((epoch + 1), (iter + 1), self.data_loader.dataset.__len__() // self.batch_size, D_loss.data[0],
                           G_loss.data[0],-D_real_loss.data[0]/self.dk,-G_loss.data[0]))
